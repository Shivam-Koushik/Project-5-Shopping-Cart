...
const register = async function (req, res) {
  try {
    ...
    if (!validator.isValid(billing.city)) return res.status(400).send({ status: false, message: "billing city is required" })
...
const login = async function (req, res) {
  try {
    ...
    if (!validator.isValidBody(req.body)) return res.status(400).send({ status: false, message: "req body is invalid !!" });
    ...
    if (!data) return res.status(401).send({ status: false, message: "email id is incorrect !" });
    // <--------password validation---------------->
    if (!password) return res.status(400).send({ status: false, message: "password is required" });
    const decryptPassword = await bcrypt.compare(password, data.password);
    if (!decryptPassword) return res.status(401).send({ status: false, message: "password is incorrect" });
...
const getProfile = async function (req, res) {
  try {
    ...
    if (!data) return res.status(400).send({ status: false, message: "Profile not found" });
